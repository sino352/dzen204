-- Command - Action
-- /st - stop all activities
-- /wb - watch the buf
-- /bb - give all bufs
-- /oop - trigger oop mode

STOPPED = true -- setting this to false will block all actions. Accessable by comand /st

AUTOBUFF = false -- setting this to true will cause script to watch all bufs. Accessable by command /wb

OOP_MODE = false -- When true bot will wait for party to give buff and will leave party after the buff

INVITE_MODE = false; -- /im

INVITE_CHAR = "Kilimanjaro" -- TODO: /im NAME
...
function IssBuff()
	...
	if not OOP_MODE or (OOP_MODE and IsInParty()) or INVITE_MODE then
		...
		if #actionsToProcess > 0 then
			if INVITE_MODE and GetUserByName(INVITE_CHAR) then
				table.insert(actionsToProcess, 1, {fnk = InviteChar, args = {INVITE_CHAR}})
			end
			if OOP_MODE then
				table.insert(actionsToProcess, {fnk = LeavePartySeq, args = {}})
			end
			table.insert(actionsToProcess, {fnk = ClearTargetSeq, args = {}}) -- don't leave players in target
			if not IsPaused() then
				SetPause(true) -- to ensure that bot will not "help" us
				table.insert(actionsToProcess, {fnk = SetPause, isLast = true, args = {false}})
			end
...
function RunCommandsByList(commandsList)
	...
	local function iterator()
		...
		-- this:Log("Bot paused " .. (IsPaused() and "true" or "false"))
	end
	iterator()
end

function InviteChar(charToInvite, callback)
	if DelayCallTask then this:Log("InviteChar() - > DelayCallTask conflict") end
	Command("/invite "..charToInvite)
	DelayCallTask = function()
		if IsInParty() then
			DelayCallTask = nil;
			DelayCallFnk(500, callback, "string") -- may be we don't need this "string"
		end
	end
end

--- set last used for all skills to 0 (so bot will cast all bufs again)
...
--- leave party and continue sequance
function LeavePartySeq(args, iterator)
	local function internalLeaveCallback()
		Command("/leave")
		if iterator then
			iterator()
		end
	end
	DelayCallFnk(2000, internalLeaveCallback)
end

function ClearTargetSeq(args, iterator)
	local function internalClearCallback()
		ClearTargets();
		ClearTarget();
		if iterator then
			iterator()
	...
	DelayCallFnk(500, internalClearCallback)
end
...
function Report()
	...
	dprint("IssByff: OOP is now " .. (OOP_MODE and "enabled" or "disabled"))
end
-----------------------------------------------------------------------------------------------------
-------------------------------------	EVENTS		-----------------------------------------------
-----------------------------------------------------------------------------------------------------
function OnCreate()
	...
	this:RegisterCommand("bb", CommandChatType.CHAT_ALLY, CommandAccessLevel.ACCESS_ME);
	this:RegisterCommand("st", CommandChatType.CHAT_ALLY, CommandAccessLevel.ACCESS_ME);
	this:RegisterCommand("wb", CommandChatType.CHAT_ALLY, CommandAccessLevel.ACCESS_ME);
	this:RegisterCommand("stat", CommandChatType.CHAT_ALLY, CommandAccessLevel.ACCESS_ME);
	this:RegisterCommand("oop", CommandChatType.CHAT_ALLY, CommandAccessLevel.ACCESS_ME);
	STOPPED = true
	AUTOBUFF = false
	ResetWatchedBufs()
end
...
function OnCommand_oop(vCommandChatType, vNick, vCommandParam)
	OOP_MODE = not OOP_MODE
	Report()
end

function OnCommand_bb(vCommandChatType, vNick, vCommandParam)
	if STOPPED then return end
	local buffSeq = {
		{fnk = CastPersonalBuffs, args = {}},
		{fnk = CastSkillsByList, args = {BUFF_20MIN}},
		{fnk = CastSkillsByList, args = {BUFF_5MIN}},
		{fnk = ClearTargetSeq, args = {}} -- don't leave players in target
	}
	if not IsPaused() then
		SetPause(true)
		table.insert(buffSeq, {fnk = SetPause, isLast = true, args = {false}})
	end
...
function OnCommand_wb(vCommandChatType, vNick, vCommandParam)
	if not ENABLED then return end
	ResetWatchedBufs()
